...
$app->DB->Update(
  ...
  WHERE mutex = 1 AND (parameter = 'api_uebertragungen') AND aktiv = 1"
);
if(!$app->DB->Select("SELECT id FROM prozessstarter WHERE mutex = 0 AND parameter = 'api_uebertragungen' AND aktiv = 1")) {
  return;
}
...
if(!class_exists('Api')){
  include_once dirname(__DIR__) . '/www/pages/api.php';
}
if(!class_exists('Uebertragungen')){
  include_once dirname(__DIR__) . '/www/pages/uebertragungen.php';
}

$api = new Api($app, true);
$uebertragungen = new Uebertragungen($app, true);
$uebertragungen_accounts = $app->DB->SelectArr(
  "SELECT * FROM uebertragungen_account WHERE aktiv = 1 AND (api > 0 OR typ = 'lokal')"
);

$belegtypen = $uebertragungen->getDocumentTypes();
$belegabkuerzungen = $uebertragungen->getShortCodes();
if(empty($uebertragungen_accounts)) {
  $app->DB->Update(
    "UPDATE prozessstarter 
  ...
  //Requests generieren  
  $app->erp->ProzessstarterStatus(
    'Requests generieren',
    $app->DB->Select("SELECT id FROM prozessstarter WHERE parameter = 'api_uebertragungen' AND aktiv = 1 LIMIT 1")
  );
  $uebertragungen->cronjobGenerateRequests($uebertragungen_accounts);


  $app->erp->ProzessstarterStatus('Artikel übertragen');
  //Artikel übertragen
  $app->DB->Update(
    "UPDATE prozessstarter SET letzteausfuerhung=NOW(), mutex = 1,mutexcounter=0 WHERE parameter = 'api_uebertragungen'"
  );

  foreach($uebertragungen_accounts as $uebertragung_account) {
    $uebertragungen->datei_id = null;
    if(method_exists($app->erp, 'canRunCronjob') && !$app->erp->canRunCronjob(['api_uebertragungen'])) {
      return;
    }
  ...
  //SalesReports uebertragen
  foreach($uebertragungen_accounts as $uebertragungen_account) {
    $uebertragungen->datei_id = null;
    if(!$uebertragungen_account['send_sales_report']) {
      continue;
    }
    $salesReports = $uebertragungen->getSellingReports($uebertragungen_account['id']);
    if(empty($salesReports)) {
      continue;
    }
    $uebertragungen->sendSellingReport($uebertragungen_account, $salesReports);
    $uebertragungen->datei_id = null;
  }
  if(method_exists($app->erp, 'canRunCronjob') && !$app->erp->canRunCronjob(['api_uebertragungen'])) {
    return;
  ...
  //Requests senden
  $app->erp->ProzessstarterStatus('Requests senden');
  foreach($uebertragungen_accounts as $uebertragung_account) {
    $uebertragungen->datei_id = null;
    $uebertragungen->cronjobSendDocuments($uebertragung_account, $api);
    $uebertragungen->datei_id = null;
  }
  if(method_exists($app->erp, 'canRunCronjob') && !$app->erp->canRunCronjob(['api_uebertragungen'])) {
    return;
  ...
  //Daten Holen
  $app->erp->ProzessstarterStatus('Daten Holen');
  $anz = 0;
  foreach($uebertragungen_accounts as $uebertragung_account) {
    $uebertragungen->datei_id = null;
    $uebertragungen->CronjobGetData($uebertragung_account, $api);
    $uebertragungen->datei_id = null;
  }
  if(method_exists($app->erp, 'canRunCronjob') && !$app->erp->canRunCronjob(['api_uebertragungen'])) {
    return;
  ...
  $app->DB->Update(
    "UPDATE prozessstarter 
    SET letzteausfuerhung= DATE_ADD(NOW(),INTERVAL 3 MINUTE), mutex = 1,mutexcounter=0 
    WHERE parameter = 'api_uebertragungen'"
  );
  $fnr3 = 1;
  foreach($uebertragungen_accounts as $uebertragung_account) {
    $uebertragungen->datei_id = null;
    $lagerzahlen = $uebertragung_account['lagerzahlen'];
    $tracking = $uebertragung_account['tracking'];

    $uebertragungen->cronjobTracking($uebertragung_account, $api);

    $uebertragungen->cronjobSendStorage($uebertragung_account, $api);

    if(empty($uebertragungen->xml_pdfselorig[$uebertragung_account['xml_pdf']])) {
      try {
        $obj = $uebertragungen->LoadTransferModul($uebertragung_account['xml_pdf'], $uebertragung_account['id']);
        if(!empty($obj) && method_exists($obj, 'generalRunning')){
          $obj->generalRunning($uebertragungen);
        }
      }
      catch(Exception $e) {
        $app->erp->LogFile(
  ...
  if(method_exists($uebertragungen,'forceAutoTracking')) {
    foreach ($uebertragungen_accounts as $uebertragung_account) {
      if($uebertragung_account['autoshopexport'] <= 0
        || empty($uebertragung_account['trackingeingang'])
        || empty($uebertragung_account['aktiv'])
        || !in_array($uebertragung_account['belegtyp'], ['lieferschein', 'auftrag'])
      ) {
        continue;
      }
      $uebertragungen->forceAutoTracking($uebertragung_account);
      $app->DB->Update(
        "UPDATE prozessstarter 
    ...
    //Ende Artikel übertragen

  if(isset($anz) && $anz > 1) {
    $app->DB->Update("UPDATE prozessstarter SET letzteausfuerhung=DATE_ADD(NOW(), INTERVAL 5 MINUTE), mutex = 0,mutexcounter=0 WHERE parameter = 'api_uebertragungen'");
  }
  else{
    $app->DB->Update("UPDATE prozessstarter SET letzteausfuerhung=NOW(), mutex = 0,mutexcounter=0 WHERE parameter = 'api_uebertragungen'");
  }

//$app->erp->ProzessUnlock($fp);
