		...
		//	eventDrop: function(event, element) 
			{
				...
				if(event.id > -1){
				$.get('./index.php?module=aufgaben&action=dragdropaufgabe&id='+event.id+'&start='+$.fullCalendar.formatDate(event.start,'yyyy-MM-dd HH-mm-ss')
...
$(function() {
	$.fn.SetFormData = function(id, start, end, allDay,task,task2) {
		if(task > 0) {
      AufgabenEdit(task);
			//window.open('index.php?module=aufgaben&action=edit&id='+task+'&back=kalender','_blank');
			return false;
		}
		else if(task2 > 0) {
			//alert("Eintrag kann nicht editiert werden");
			AufgabenEdit(task2);
			//window.open('index.php?module=aufgaben&action=edit&id='+task2+'&back=kalender','_blank');
			return false;
		}
    else if(id > -1) {
      AufgabenEdit(id);
			//window.open('index.php?module=aufgaben&action=edit&id='+id,'_blank');
      return false;
		}else{
      //window.open('index.php?module=aufgaben&action=edit&id='+id,'_blank');
			return false;
