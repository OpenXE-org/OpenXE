...
<!--    <div class="mlmClear"></div>  -->
    ...
    $(document).ready(function() {
      ...
      $('#mlmTree').aciTree({
          ...
          checkboxChain: false,
          ajax: {
              url: 'index.php?module=databaseviewer&action=treeajax&cmd=suche'
          },
          checkbox: true,
      ...
      $('#mlmTree').on('acitree', function(event, api, item, eventName, options){
          switch (eventName){
              ...
              case 'selected':
                ...
                $.ajax({
                  url: 'index.php?module=databaseviewer&action=list&cmd=filtertree&withgui=1',
                  data: ajaxData,
                  type: 'POST',
                  dataType: 'json',
                  success: function(data) {
                      if(typeof data.html != 'undefined') {
                          $('div#table').html(data.html);
                          $('div#table').find('#textvorlagenModal').remove();
                      }
                  }
                });
