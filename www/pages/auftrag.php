    ...
    $menu ="
      ...
      function onchangeauftrag(cmd)
      {
        switch(cmd)
        {
          ...
          $proformarechnungcase
          $hookcase
          case 'etiketten': window.location.href='index.php?module=auftrag&action=etiketten&id=%value%';
        }
      }
    </script>
...
//      $this->app->Tpl->Set('MESSAGE','<div class="error">Cronjob order processing not yet implemented!</div>');
  ...
  */
  ...
  function AuftragOffenePositionen() {
    ...
    function Kommissionieren(int $kommissionierung, int $auftrag, int $lieferschein, bool $ausgelagert, array $lagerplatzliste, int $mengedruck, $druckercode, string $kommentar) {
