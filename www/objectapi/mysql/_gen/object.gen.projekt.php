...
class ObjGenProjekt
{
  ...
  private  $buchhaltung_berater;
  private  $buchhaltung_mandant;
  private  $buchhaltung_wj_beginn;
  private  $buchhaltung_sachkontenlaenge;
  ...
  private  $standardkommissionierlagerplatz;
...
$result = $result[0];
    ...
    $this->buchhaltung_berater=$result['buchhaltung_berater'];
    $this->buchhaltung_mandant=$result['buchhaltung_mandant'];
    $this->buchhaltung_wj_beginn=$result['buchhaltung_wj_beginn'];
    $this->buchhaltung_sachkontenlaenge=$result['buchhaltung_sachkontenlaenge'];
    ...
    $this->standardkommissionierlagerplatz=$result['standardkommissionierlagerplatz'];
  ...
  public function Create()
  {
    ...
    '{$this->buchhaltung_berater}',
    '{$this->buchhaltung_mandant}',
    '{$this->buchhaltung_wj_beginn}',
    '{$this->buchhaltung_sachkontenlaenge}',

         '{$this->create_proformainvoice}','{$this->print_proformainvoice}','{$this->proformainvoice_amount}','{$this->anzeigesteuerbelegebestellung}','{$this->autobestbeforebatch}','{$this->allwaysautobestbeforebatch}','{$this->kommissionierlauflieferschein}','{$this->intraship_exportdrucker}','{$this->multiorderpicking}','{$this->standardlager}','{$this->standardkommissionierlagerplatz}','{$this->standardlagerproduktion}','{$this->klarna_merchantid}','{$this->klarna_sharedsecret}','{$this->nurlagerartikel}','{$this->paketmarkedrucken}','{$this->lieferscheinedrucken}','{$this->lieferscheinedruckenmenge}','{$this->auftragdrucken}','{$this->auftragdruckenmenge}','{$this->druckennachtracking}','{$this->exportdruckrechnungstufe1}','{$this->exportdruckrechnungstufe1menge}','{$this->exportdruckrechnung}','{$this->exportdruckrechnungmenge}','{$this->kommissionierlistestufe1}','{$this->kommissionierlistestufe1menge}','{$this->fremdnummerscanerlauben}','{$this->zvt100url}','{$this->zvt100port}','{$this->production_show_only_needed_storages}','{$this->produktion_extra_seiten}','{$this->kasse_button_trinkgeldeckredit}','{$this->kasse_autologout}','{$this->kasse_autologout_abschluss}','{$this->next_receiptdocument}','{$this->taxfromdoctypesettings}')";
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE `projekt` SET
      ...
      `buchhaltung_berater`='{$this->buchhaltung_berater}',
      `buchhaltung_mandant`='{$this->buchhaltung_mandant}',
      `buchhaltung_wj_beginn`='{$this->buchhaltung_wj_beginn}',
      `buchhaltung_sachkontenlaenge`='{$this->buchhaltung_sachkontenlaenge}',
      ...
      `standardkommissionierlagerplatz`='{$this->standardkommissionierlagerplatz}',
...
//    echo($sql);
  ...
  public function Delete($id='')
  {
    ...
    $this->standardlager='';
    $this->standardkommissionierlagerplatz='';
 ...
 /** dank dieser funktionen kann man die tatsaechlichen werte einfach 
  ...
  public function SetBuchhaltung_Berater($value) { $this->buchhaltung_berater=$value; }
  public function GetBuchhaltung_Berater() { return($this->buchhaltung_berater); }
  public function SetBuchhaltung_Mandant($value) { $this->buchhaltung_mandant=$value; }
  public function GetBuchhaltung_Mandant() { return($this->buchhaltung_mandant); }
  public function SetBuchhaltung_Wj_Beginn($value) { $this->buchhaltung_wj_beginn=$value; }
  public function GetBuchhaltung_Wj_Beginn() { return($this->buchhaltung_wj_beginn); }
  public function SetBuchhaltung_Sachkontenlaenge($value) { $this->buchhaltung_sachkontenlaenge=$value; }
  public function GetBuchhaltung_Sachkontenlaenge() { return($this->buchhaltung_sachkontenlaenge); }
  ...
  public function SetStandardkommissionierlagerplatz($value) { $this->standardkommissionierlagerplatz=$value; }
  public function GetStandardkommissionierlagerplatz() { return $this->standardkommissionierlagerplatz; }
