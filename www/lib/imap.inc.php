...
class IMAP
{
  public $IMAP_IMAP;
  public $IMAP_POP3;
  public $IMAP_IMAP_SSL;
  public $IMAP_POP3_SSL;
  public $attachments;
  public $htmlmsg;
  public $plainmsg;
  public $charset;
  ...
  function imap_serverpath($server, $port, $folder, $type)
  {
    ...
    switch ($type) {
      case $this->IMAP_IMAP:
        $server_path = '{' . $server . ':' . $port . '/novalidate-cert}' . $folder;
        break;
      case $this->IMAP_POP3:
        $server_path = '{' . $server . ':' . $port . '/pop3}' . $folder;
        break;
      case $this->IMAP_IMAP_SSL:
        $server_path = '{' . $server . ':' . $port . '/imap/ssl/novalidate-cert}' . $folder;
        break;
      case $this->IMAP_POP3_SSL:
        $server_path = '{' . $server . ':' . $port . '/pop3/ssl}' . $folder;
        break;
      default:
        $server_path = '{' . $server . ':' . $port . '}' . $folder;
        break;
    }
    return $server_path;
  }
