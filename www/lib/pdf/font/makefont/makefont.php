...
function SaveToFile($file, $s, $mode, $path="")
{
	$f = fopen($path.$file, 'w'.$mode);
	if(!$f)
		Error('Can\'t write to file '.$path.$file);
	fwrite($f, $s);
	fclose($f);
}
function MakeDefinitionFile($file, $type, $enc, $embed, $subset, $map, $info, $path = "")
{
	$s = "<?php\n";
	$s .= '$type = \''.$type."';\n";
...
{
	$pathtmp = substr($fontfile, 0,strrpos($fontfile, "/"));
	$path = substr($pathtmp, 0,strrpos($pathtmp, "/")+1);
	...
	if($embed)
	{
		...
		{
			$file = $basename.'.z';
			SaveToFile($file, gzcompress($info['Data']), 'b',$path);
			$info['File'] = $file;
	...
	MakeDefinitionFile($basename.'.php', $type, $enc, $embed, $subset, $map, $info, $path);
