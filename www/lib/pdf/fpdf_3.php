...
class FPDFWAWISION 
{
  ...
  {
    //if($family=="DejaVu" || $family=="dejavu")
    //  $family = "DejaVuSansCondensed";
    
    $style = strtoupper($style);
    $filettf = $this->getFontFiles($family.$style,'ttf');
    $familyttf = $this->getFontFiles($family,'ttf');
    if(is_file($this->_getfontpath()."/unifont/".$filettf.".ttf"))
    {
      $uni=true;
      $file=$filettf.".ttf";
    }
    else if (is_file($this->_getfontpath()."/unifont/".$familyttf.".ttf"))
    {
      $style="";
      $uni=true;
      $file=$familyttf.".ttf";
    ...
    if($file=='') {
      if ($uni) {
        $file = $this->getFontFiles(str_replace(' ','',$family).strtolower($style)).'.ttf';
        //$file = str_replace(' ','',$family).strtolower($style).'.ttf';
      }
      else {
        $file = $this->getFontFiles(str_replace(' ','',$family).strtolower($style),'php').'php';
    ...
    if ($uni) {
      ...
      $unifile2 = $this->getFontFiles($unifilename,'uniphp');
      if (file_exists($unifile2.'.mtx.php')) {
        include($unifile2.'.mtx.php');
      }
  ...
  function SetFontClassic($family,$style='',$size=0){
    $this->SetFont($family,$style,$size);
  }

  protected function getFontFiles($file, $type = 'ttf')
  {
    if($type === 'ttf' && file_exists($this->_getfontpath().'unifont/'.$file.'.ttf'))
    {
      return $file;
    }elseif($type === 'php' && file_exists($this->_getfontpath().$file.'.php'))
    {
      return $file;
    }elseif($type === 'uniphp' && file_exists($this->_getfontpath().'unifont/'.$file.'.php'))
    {
      return $file;
    }
    if($type === 'ttf' && !empty($this->fontsttf[strtolower($file)]))
    {
      return $this->fontsttf[strtolower($file)];
    }elseif($type === 'php' && !empty($this->fontsphp[strtolower($file)]))
    {
      return $this->fontsphp[strtolower($file)];
    }
    elseif($type === 'uniphp' && !empty($this->fontsuniphp[strtolower($file)]))
    {
      return $this->fontsunuphp[strtolower($file)];
    }

    if($handle = opendir($this->_getfontpath().'unifont/'))
    {
      while($entry = readdir($handle))
      {
        if(strpos($entry,'.') !== 0 && is_file($this->_getfontpath().'unifont/'.$entry) && strtolower(substr($entry,-4)) === '.ttf')
        {
          $this->fontsttf[strtolower(substr($entry,0,-4))] =  substr($entry,0,-4);
        }
      }
      closedir($handle);
    }
    if($handle = opendir($this->_getfontpath()))
    {
      while($entry = readdir($handle))
      {
        if(strpos($entry,'.') !== 0 && is_file($this->_getfontpath().$entry) && strtolower(substr($entry,-4)) === '.php')
        {
          $this->fontsphp[strtolower(substr($entry,0,-4))] =  substr($entry,0,-4);
        }
      }
      closedir($handle);
    }
    if($handle = opendir($this->_getfontpath().'unifont/'))
    {
      while($entry = readdir($handle))
      {
        if(strpos($entry,'.') !== 0 && is_file($this->_getfontpath().'unifont/'.$entry) && strtolower(substr($entry,-4)) === '.php')
        {
          $this->fontsuniphp[strtolower(substr($entry,0,-4))] =  substr($entry,0,-4);
        }
      }
      closedir($handle);
    }
  ...
  function SetFont($family, $style='', $size=0)
  {
    ...
    if(strpos($family, ' ') || strpos($family, '.') || strpos($family, ',')){
      $family = 'DejaVuSansCondensed';//"arial";
    }
    ...
    if($family=='times' or $family=='helvetica'){
      $fontfile .= strtolower($style);
    }

    // begin updated code

    $fontfilephp = $this->getFontFiles($fontfile, 'php');


    if(!file_exists($this->_getfontpath().$fontfilephp.'.php')){
      $fontfilettf = $this->getFontFiles($fontfile);
      if(file_exists($this->_getfontpath().'unifont/'.$fontfilettf.'.ttf')){
        exec("php ".$this->_getfontpath().'makefont/makefont.php '.$this->_getfontpath()."unifont/".$fontfilettf.'.ttf', $output);
        $fontfilephp = $this->getFontFiles($fontfile, 'php');
        if(!isset($this->CoreFonts[$fontkey])){
          if(file_exists($this->_getfontpath().$fontfilephp.'.php')){
            $this->AddFont($family,$style,$fontfile.'.php');  
          }else{
            //$this->Error("Fontfile konnte nicht erstellt werden: ".$fontfile.".php\n". print_r($output,true));
          }
        }
      }else{
        //Fallback auf letzt verwendete Font
        //$this->Error('Necessary '.$fontfilettf.'.ttf file not found. See the Xentral helpdesk for more information about custom fonts.');
      }
    }else{
      if(!isset($this->CoreFonts[$fontkey])){
        $this->AddFont($family,$style,$fontfile.'.php');  
    ...
    if(!isset($this->fonts[$fontkey]))
    {
      $fontkey2 = $this->getFontFiles($fontkey, 'php');

      if(file_exists($this->_getfontpath().$fontkey2.'.php'))
      {
        include $this->_getfontpath().$fontkey2.'.php';
      }else{
        for($i=0;$i<=255;$i++) {
          $fpdf_charwidths[$fontkey][chr($i)] = 600;
        }
      }
  ...
  {
    if(!isset($this->ttffontuni) || $this->ttffontuni === false) {
  ...
  function _getpagesize($size)
  {
    ...
    {
      // keine automatische Drehung BS 05.06.2018
      //if($size[0]>$size[1])
  ...
  function _parsegif($file)
  {
    ...
    else
    {
      ...
      $tmp = tempnam('.','gif');
      if(!$tmp)
        $this->Error('Unable to create a temporary file: ' . $tmp);
      if(!imagepng($im,$tmp))
        $this->Error('Error while saving to temporary file: ' . $tmp);
      imagedestroy($im);
  ...
  function _putfonts()
  {
    ...
    foreach($this->FontFiles as $file=>$info)
    {
      if (!isset($info['type']) || $info['type']!='TTF') {
        ...
        if(!$f)
          $this->Error('Font file not found: ' . $this->_getfontpath() . $file);
        while(!feof($f))
          $font.=fread($f,8192);
        fclose($f);
