...
var PDFViewerApplication = {
  ...
  open: function open(file, args) {
    ...
    return loadingTask.promise.then(function (pdfDocument) {
      ...
      var loadingErrorMessageCustom = null;
      if (exception instanceof _pdfjsLib.InvalidPDFException) {
        loadingErrorMessage = _this3.l10n.get('invalid_file_error', null, 'Invalid or corrupted PDF file.');
        loadingErrorMessageCustom = 'PDF-Datei ist ungültig oder beschädigt (Invalid or corrupted PDF file).';
      } else if (exception instanceof _pdfjsLib.MissingPDFException) {
        loadingErrorMessage = _this3.l10n.get('missing_file_error', null, 'Missing PDF file.');
        loadingErrorMessageCustom = 'PDF-Datei fehlt (Missing PDF file).';
      } else if (exception instanceof _pdfjsLib.UnexpectedResponseException) {
        loadingErrorMessage = _this3.l10n.get('unexpected_response_error', null, 'Unexpected server response.');
        loadingErrorMessageCustom = message;
      } else {
        loadingErrorMessage = _this3.l10n.get('loading_error', null, 'An error occurred while loading the PDF.');
        if (message === 'PDFDocument: stream must have data') {
          loadingErrorMessageCustom = 'PDF-Datei ist leer (Stream must have data).';
        }
      }

      if (loadingErrorMessageCustom !== null) {
        alert(loadingErrorMessageCustom);
      }
