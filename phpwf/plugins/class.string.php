...
class WawiString
{
    public function Convert($value, $input, $output)
    {
        if ($input == '')
            return $value;
    ...
    private function BuildRegExp(array $array): string
    {
        $regexp = '/^';
    ...
    private function FindPercentValues(string $pattern): array
    {
    ...
    public function decodeText($_str, $_form = true)
    {
        if ($_form) {
            $_str = str_replace('#BR#', "\r\n", $_str);
        } else {
    ...
    public function fixeUmlaute(mixed $text): mixed
    {
        if (!is_string($text)) {
            return $text;
        }
    ...
    private function getUmlauteArray(): array
    {
        return [
            'Ã¼' => 'ü',
            'Ã¤' => 'ä',
    ...
    public function unicode_decode(string $content): string
    {
        $ISO10646XHTMLTrans = [
            '&' . '#34;' => '&quot;',
            '&' . '#38;' => '&amp;',
    ...
    public function ReadyForPDF(string $string = ''): string
    {
        return trim(
            html_entity_decode(
                str_replace(
